const day_03_1 = () => {
    ...
    const run = () => {
        ...
        for (var i = 0; i < data.length; i++) {
            const line = data[i];
            lines.push([line.substring(0, line.length / 2), line.substring(line.length / 2)]);
        }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const compartment1 = line[0];
            const compartment2 = line[1];

            const priorities = [];

            for (let j = 0; j < compartment1.length; j++) {
                if (compartment2.indexOf(compartment1[j]) < 0)
                    continue;
